...
const Home = () => {
  ...
  const fetchUser = async () => {
    try {
      const overdueRes = await axios.get(`${API}/debt/overdue`);
      
      setOverdue(overdueRes.data);

      const commonRes = await axios.get(`${API}/common/debts`, {
        headers: { Authorization: `Bearer ${cookies.token}` }
      });
      setCommon(commonRes.data);

      const clientsRes = await axios.get(`${API}/debter`);
      setClientsCount(clientsRes.data.total);

      const userRes = await axios.get(`${API}/auth/me`, {headers: {Authorization: `Bearer ${cookies.token}`}});
                ...
                <div className="flex justify-between items-center">
                  <div className="flex items-center gap-[12px]">
                    ...
                    <div className="space-y-1">
                      ...
                      <p className="flex gap-[10px] font-bold text-[24px] leading-[100%] " >{user?.balance && FormatterPrice(user.balance.toString())?.space} <p className="font-bold text-[18px] ">so'm</p></p>
                    </div>
                  </div>
                  <button onClick={() => navigate(paths.payBalance)} className="text-white bg-[#3478F7] rounded-full flex items-center justify-center w-[36px] h-[36px] "><PlusIcon/></button>
                </div>
