...
const ClientsEdit = () => {
  ...
  useEffect(() => {
    const fetchClient = async () => {
      try {
        const res = await axios.get(`${API}/debter/${id}`, {
        ...
        const imgRes = await axios.get(`${API}/images-debt?debtId=${id}`, {headers: {Authorization: `Bearer ${cookies.token}`}})
        console.log(imgRes);
        

        if (res.data.images && Array.isArray(res.data.images)) {
          const loadedPreviews = ["", ""];
          res.data.images.forEach((img: any, idx: number) => {
            if (idx < 2) loadedPreviews[idx] = img.image;
          });
          setPreviews(loadedPreviews);
        }
      } catch (err) {
        toast.error("Mijoz ma'lumotlarini olishda xatolik");
      }
    };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (id) {
        await axios.patch(
          `${API}/debter/${id}`,
          { name: clientName, phone, adress, note },
          { headers: { Authorization: `Bearer ${cookies.token}` } }
        );
      } else {
      ...
      toast.success(id ? "Yangilandi" : "Yaratildi");
      navigate(-1);
    } catch (error: any) {
    ...
    <div className="containers">
      <div className="flex mt-[34px] items-center justify-between px-4 py-4">
        ...
        <h1 className="text-lg font-semibold">
          {id ? "Mijozni tahrirlash" : "Yangi mijoz"}
        </h1>
      ...
      >
        {isLoading ? "Yuklanmoqda..." : id ? "Yangilash" : "Saqlash"}
      </button>
    </div>
  );
};
